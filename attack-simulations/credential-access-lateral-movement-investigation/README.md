#  Credential Access Attempts & Suspected Lateral Movement Investigation
...
| Unmonitored host |   — |  | 192.168.56.103 (Source of the Nmap scan and SSH brute-force attempts) |
...
The earliest activity captured was a full process chain showing how the first discovery command was actually launched 
![Figure 4 ](screenshots/14-process-tree-analysis.png)
*Figure 4: Wazuh recorded the chain `powershell.exe → cmd.exe /c "net user" → net.exe` (resolving to `net1 user`) on WIN10-ENDPOINT. Two matched rules — "A C:\Windows\System32\net.exe binary was started" and "Powershell process spawned Windows command shell" — confirm `net user` was launched indirectly through PowerShell rather than typed at an interactive command prompt. This is the opening move of the investigation: account enumeration, launched through a PowerShell wrapper.*

A baseline `whoami.exe` execution was then observed:

![Figure 5](screenshots/04-baseline-sysmon-activity.png)
*Figure 5: `whoami.exe` executed on WIN10-ENDPOINT (192.168.56.101) — establishing the account context for the session before the discovery activity escalated.*
...
![Figure 6](screenshots/05-account-discovery-net-exe.png)
*Figure 6: Wazuh threat-hunting results for WIN10-ENDPOINT shows `net.exe user`, followed by two `net.exe localgroup administrators` executions — each one matched by both "Discovery activity executed" and "Discovery activity spawned via powershell execution," with `powershell.exe` as the parent in every case.*

The PowerShell parent process itself was also captured directly:

![Figure 7](screenshots/12-powershell-reconnaissance.png)
*Figure 7: Filtering specifically on "Discovery activity spawned via powershell execution" (rule 92033) returns exactly four hits, matching the `net.exe` and `arp.exe` events already documented above — confirming that the discovery commands were consistently launched through PowerShell during the observed window.*

Privilege discovery followed:

![Figure 8](screenshots/06-privilege-discovery.png)
*Figure 8: `whoami.exe /priv` executed on WIN10-ENDPOINT — the account moved from asking "who am I" and "who else is here" to "what am I allowed to do."*

And finally, network configuration discovery:

Network configuration discovery followed. Wazuh captured `arp.exe -a` executing on WIN10-ENDPOINT, with `powershell.exe` recorded as the parent process. The `-a` parameter queries the local ARP cache, providing visibility into recently resolved network neighbors.

![Figure 9](screenshots/07-network-discovery.png)
*Figure 9: Wazuh process telemetry showing `arp.exe -a` executing on WIN10-ENDPOINT. The event records `powershell.exe` as the parent process, linking the ARP enumeration to the same PowerShell-driven discovery sequence.*
...
![Figure 10](screenshots/08-nmap-service-discovery.png)
*Figure 10: The captured Nmap activity originated from the unmonitored host 192.168.56.103 and targeted the Ubuntu server at 192.168.56.102. The `-sV` scan identified OpenSSH on TCP/22 and Apache HTTP on TCP/8080.
Shortly after, SSH authentication attempts began against that same server:

![Figure 11](screenshots/09-ssh-authentication-failure.png)
*Figure 11: `/var/log/auth.log` on ubuntu-soc shows repeated failed SSH logins for invalid user "beryl" from 192.168.56.103 — the same IP that ran the Nmap scan. Each attempt returned "Failed password for invalid user," "authentication failure," and "user unknown," and the client closed the connection after PAM logged two more authentication failures. No successful authentication appears in this log for the account.*

The same activity was independently confirmed in Wazuh:

![Figure 12](screenshots/10-wazuh-ssh-authentication-events.png)
*Figure 12: Wazuh's UBUNTU-SURICATA feed shows the same window, matched against rule 5710 ("Attempt to login using a non-existent user") and rule 5712 ("sshd: brute force trying to get access to the system. Non existent user"), corroborating the auth.log findings from an independent source.*
...
![Figure 13](screenshots/11-sysmon-network-connection.png)
*Figure 13: Sysmon Event ID 3 (Network Connection) telemetry on WIN10-ENDPOINT shows 2,097 hits in the surrounding 24-hour window, with rule 92106 ("Windows System process activity over SMB port - Possible suspicious access to Windows admin shares") firing repeatedly in a tight cluster directly adjacent to the failed Ubuntu SSH attempt.*

![Figure 14](screenshots/13-windows-authentication-event.png)
*Figure 14: A search for Windows Security Event ID 4625 on WIN10-ENDPOINT returns 15 hits, with a cluster of seven landing in the same few minutes as the SMB spike in Figure 11 — all matched to rule 60122, "Logon Failure - Unknown user or bad password."*

**Assessment of Phase 3:** the timing is tight enough to be worth flagging — SMB port activity and Windows logon failures cluster in the same four-minute window that immediately follows the failed SSH brute-force on the Ubuntu server. That said, **neither Figure 11 nor Figure 13 surfaces a source IP field**, so this correlation is temporal only. It is documented as a plausible attempted pivot from the same source host toward the Windows endpoint, not a confirmed one. No Event ID 4624 (successful logon) was found in this window, so even if this was a pivot attempt, it did not result in confirmed access.
...
## Attack Timeline
 | Host | Activity |
 |---|---|
 | ubuntu-soc | Wazuh agent confirmed active (pre-investigation check) |
 | WIN10-ENDPOINT | `powershell.exe → cmd.exe → net.exe` — `net user` (process chain captured) |
 | WIN10-ENDPOINT | Baseline `whoami.exe` |
 | WIN10-ENDPOINT | `net user` via PowerShell |
 | WIN10-ENDPOINT | `net localgroup administrators` (x2) via PowerShell |
 | WIN10-ENDPOINT | `whoami /priv` — privilege discovery |
 | WIN10-ENDPOINT | `arp -a` (x3) — network discovery |
 | WIN10-ENDPOINT | Final PowerShell-spawned discovery event |
 | 192.168.56.103 → .102 | Nmap `-sV` scan — ports 22 (SSH) and 8080 (HTTP) found open |
 | 192.168.56.103 → ubuntu-soc (.102) | Repeated failed SSH logins, invalid user "beryl" |
 | WIN10-ENDPOINT | Cluster of 4625 logon failures |
 | WIN10-ENDPOINT | Spike of SMB-port network connections (rule 92106) |
...
| SMB-port network activity | T1046 – Network Service Discovery / reconnaissance context |
...
**Final determination:** The investigation identified a sequence of endpoint discovery, network reconnaissance, and credential-access attempts. The Nmap scan and SSH authentication attempts were directly correlated to the same source host, while the subsequent Windows SMB and logon-failure activity was temporally correlated but could not be source-attributed with the available telemetry. No successful authentication or confirmed lateral movement was identified.
